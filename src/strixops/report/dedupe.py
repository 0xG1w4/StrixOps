"""SDK-native vulnerability deduplication adapted from Strix v1.6.0.

The caller supplies the task's model route. Comparison rules, response parsing,
and fail-open behavior follow the reference's dynamic vulnerability reports.
"""
...
async def check_duplicate(
    ...
    *,
    resolve_model: Callable[[], Model] | None = None,
) -> dict[str, Any]:
    ...
    try:
        ...
        comparison_data = {
            "candidate": _prepare_report_for_comparison(candidate),
            "existing_reports": [_prepare_report_for_comparison(r) for r in existing_reports],
        }
        user_msg = (
        ...
        # The injected UsageTrackingModel accounts for this non-streaming call.
        ...
        return {
            ...
            "duplicate_id": "",
            "confidence": 0.0,
            "reason": f"Deduplication check failed: {exc}",
            "error": str(exc),
        }
    else:
        return result
