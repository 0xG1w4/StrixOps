---
name: api_attacks
description: API attack playbook — business-logic abuse via API chaining, GraphQL batching, pagination exploitation, webhook hijacking, API races, key-leak patterns, undocumented endpoint discovery, REST verb tampering, gRPC and WebSocket testing, versioning misconfiguration. Object/field authorization depth lives in vulnerabilities/idor, broken_function_level_authorization and mass_assignment.
---

API attack flows beyond object-level authorization: chaining, batching, and
protocol-level abuse. For BOLA/IDOR use `vulnerabilities/idor`; for function
and field authorization use `vulnerabilities/broken_function_level_authorization`
and `vulnerabilities/mass_assignment`; for token attacks use
