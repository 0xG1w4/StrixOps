...
``exec_command`` shell tool. When Caido is active for a web scan,
``http_proxy`` / ``https_proxy`` point to its HTTP/HTTPS proxy.
The installed agent-browser reads these variables automatically, so
``--proxy`` is unnecessary. HTTP/HTTPS page requests routed through the
proxy appear in the built-in ``list_requests`` tool. Localhost traffic,
including CDP, bypasses the proxy via ``NO_PROXY=localhost,127.0.0.1``.
