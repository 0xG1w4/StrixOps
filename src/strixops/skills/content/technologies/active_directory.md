...
The shared web/internal sandbox includes Impacket, Certipy, BloodHound CE, and core SMB/LDAP/Kerberos tools. NetExec and enum4linux-ng installation is best-effort and may be unavailable; check commands with `command -v` before use. Use the installation commands below only for missing tools needed by the task. AD testing also requires **network reachability to the target DC/subnet**; confirm connectivity first.
