...
async def run_scan(spec: ScanSpec, settings: EngineSettings) -> int:
    ...
    try:
        ...
        async def finalize() -> None:
            ...
            # directly. Keep a readable draft even if the first cleanup stalls.
            try:
            ...
            try:
                ...
                # Refresh the draft before the model produces the final report.
                # A model failure retains source material clearly labeled as a
                # draft, never as a successfully synthesized final report.
                run_state.write_executive_report()
                run_state.save()
                synthesized = None
    ...
    if scan_succeeded and not cleanup_errors:
        ...
        _stdout_log(
            "Final report generated by model."
            if run_state.run_record.get("report_synthesized") is True
            else "Final model report not generated; saved content is a draft."
        )
        return EXIT_OK
    return EXIT_FAILED
