...
點選一筆流量可查看 request／response、重複 headers、body、HTTP 版本、狀態與耗時。gzip、deflate 與 Brotli 內容會先解壓，再依宣告的文字編碼轉成可讀內容；JSON 可切換格式化顯示。前端與 Agent 使用同一份解碼邏輯，原始 headers 與 body bytes 繼續保存，未修改的重送仍使用原始資料。完整保存的舊流量也能使用新的解碼檢視，不需要先重新捕獲。解碼失敗、串流尚未收完、截斷內容與真正二進位格式會分別標示。預設遮罩常見憑證欄位；需要檢視原始內容時，可使用介面的揭露操作。遮罩是顯示層行為，原始證據仍保存在此主機的任務資料中。
...
手動「重送請求」可修改方法、同來源 URL、一般 headers 及 body。傳輸層管理 Host、Content-Length、Connection 等 framing／proxy headers；不提供 CONNECT、原始 request smuggling 或跨來源攜帶登入憑證的重送。原始 request body 尚未完成或已被截斷時，不能重送。以文字編輯 body 時，新內容使用 UTF-8，傳輸層會移除繼承的 Content-Encoding 並同步 Content-Type charset；若需送出已編碼 bytes，使用 API 的 body_base64 與一致的標頭。
...
- 設定請求次數與時間預算。新任務預設最多 12 次重送、300 秒；每次工作最多選取 50 筆原始請求。所有選取共用整次預算，時間包含模型回覆、工具操作及重送。12 次是可使用的上限，不要求一定執行完 12 次。已保存的任務設定（例如 120 秒）會繼續沿用，可在建立測試時調整；上限仍為 900 秒。
- 執行後查看進度、測試產生的流量、覆蓋記錄、finding 與未完成原因。

模型設定檔和 skill 內容共用既有管理介面；每次 TestJob 會保存當時的 prompt／skill 快照及雜湊。任務預設設定之後的變更不會改寫已建立的測試快照。

首輪會提供有界、已遮罩的 request／response 證據與技能目錄，內容足夠時可直接判斷並重送。省略或縮短的內容會標示，必要時仍需 inspect_request。預設使用簡短的 HTTP 證據與嚴重度準則，完整基礎技能保留為按需載入；明確選取的技能仍會全文載入。已載入技能有標記，重複載入不再追加全文。

測試使用串流，畫面顯示模型活動、實際工具名稱、每回合首個事件／輸出耗時，以及供應商有提供的 token 用量與結束原因。首個事件可能只是連線已有回應；首個模型輸出包含推理、文字或工具參數活動，均不代表重送已完成。診斷不顯示原始推理或工具參數，缺少的統計會維持未知。MCP 不再額外指定 2,500-token 輸出上限，沿用設定檔的 API 與 reasoning 選項；default 不等於強制關閉模型思考。

Agent 會收到剩餘時間與重送額度，並預留整次時間的 30%（最多 60 秒）用於整理結果。進入收尾後不再啟動新重送；未完成的檢查標為 needs_follow_up，畫面以「部分完成」區分。達到硬性期限時則顯示「時間用盡」、當時階段及已用額度，已保存的流量、發現和覆蓋記錄繼續保留。執行紀錄包含模型回合與工具耗時；取消與容器清理可能讓實際完成時間稍長於設定期限。
...
| 捕獲 body | 每個 request／response 保存最多 256 KiB 原始前綴；解碼預覽另外限制為 256 KiB，超過會明確標示；流量仍繼續轉送 |
