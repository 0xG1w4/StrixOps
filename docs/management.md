...
任务页的重跑窗口提供「新任务」和「接续任务」，默认选择「新任务」。两种方式都会
创建独立任务，并保留原任务。「接续任务」将上一轮正式报告作为参考，可展开预览
全文，也可追加本轮指令。必须是已经结束的任务，且正式模型报告已保存、非空、可读；
草稿或来源状态不明时只能选择「新任务」。停止或失败的任务后来成功生成正式报告后，
也可以接续。重跑沿用目标、项目、访问方式与测试深度，允许编辑指令及测试深度，
使用打开窗口时当前启用的模型配置。

提交时再次校验报告版本，报告已更新则提示刷新后重试。新任务保存独立的
`previous_report.md` 副本，排队后原报告变更或删除不会改变此次输入。完整报告仅在
Root 的首次任务输入中提供，子代理由 Root 分派相关摘录；报告不会写入
`instruction.md`，也不会自动导入旧对话、漏洞记录或证据附件。再次接续只引用直接
上一轮的正式报告。任务页显示来源任务链接；完整输入超过所选模型的估算容量时拒绝
启动，保留全文，不会静默截断。模型容量来自本机模型资料，未知模型使用配置的
`STRIX_CONTEXT_FALLBACK_TOKENS`；离线计数无法获得 tokenizer 时采用保守估算。

设置的模型路由中可单独选择「报告模型」，并指定该模型的 API 类型与推理强度，
与 Web／内网模型共用该路由的 API 地址和密钥。报告模型留空时，完整沿用任务模型、
API 类型和推理强度。自动报告使用启动任务时保存的设置，批次任务也会保存此配置；
更改设置不会改变已经运行或排队任务的自动报告模型。
手动生成使用当前保存的模型设置：优先使用该任务原先选择的 profile 的报告模型，
若该 profile 已删除则使用当前启用的 profile；报告模型留空时沿用对应扫描类型的模型。
原始扫描设置与完成状态保持不变，成功生成所使用的模型记录于 `run.json` 的
`report_generated_model`。来源容量按照实际报告模型计算。
CLI 可用 `STRIX_REPORT_LLM`、`REPORT_LLM_API_MODE`、`REPORT_LLM_REASONING_EFFORT`
设置报告模型及选项；未指定 `STRIX_REPORT_LLM` 时沿用扫描模型的全部设置。
...
表头并只提取结构化凭据行，与登记清单及明确的结构化凭据字段去重后送入报告。
...
界面、CSV 下载和报告共用登记清单，并使用明确的结构化凭据字段及凭据 CSV 补漏；同一凭据的当前登记状态
优先，旧来源的不同验证观察保留为补充资料。CSV 在下载时从统一清单生成，无需 Agent
在沙箱中维护另一份文件，也不增加报告模型的抽取调用。
结构化字段必须保存为实际的对象或列表，例如 finding 的 `metadata.credentials`。
普通发现或笔记正文、URL 示例、Markdown 表格或代码块，以及正文中的 JSON 字符串
都不会被推测成凭据条目。漏洞、内部发现、共享笔记、测试覆盖、威胁模型及收尾结论
仍依原规则作为报告上下文。
凭据清单也不会读取模型设置中的 API 密钥。重复项保留全部来源；发现某个
...
升级后重新读取清单时，以前仅从普通文字推测出的条目不再出现；不会删除登记库中的
记录。已保存的报告保持原样，需要在任务页点击「生成报告」才会使用新的来源规则。
