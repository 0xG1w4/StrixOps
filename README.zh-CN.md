...
稳定版本 **1.3.5** · [版本说明](docs/v1.3.5.md) · [更新日志](CHANGELOG.md) · [Apache-2.0](LICENSE)

持续维护的稳定分支为 **[`v1.3.5-release`](https://github.com/0xG1w4/StrixOps/tree/v1.3.5-release)**，
以应用代码 `85d980b` 为基底，保留该基线的扫描引擎、提示词和技能内容，同时回补 `93f503f` 的控制台任务选择、项目筛选与批量删除功能，版本维持 1.3.5。
`v1.3.5` 标签保留为回补之前的固定历史快照；请拉取 **`v1.3.5-release` 分支**获取新增功能。
**`main` 和 `v1.4.0-dev` 属于开发版本**。已纳入与未纳入的改动见[稳定基线范围](docs/v1.3.5.md#stable-baseline-scope)。
...
| 总览任务管理 | 项目筛选、逐项选择、跨页全选筛选结果及确认后批量删除 | 审阅删除范围，并分别查看成功数量与逐项失败原因 |
...
在总览中按搜索词、扫描类型或项目筛选任务，再逐项勾选或**全选**。全选包含当前筛选结果所有分页中可选择的任务；改变筛选条件会清空选择。删除确认框列出任务名称、目标及数量，确认后永久删除所选任务保存的报告、证据和日志。结果分别显示成功数量与每个失败任务的原因。活动引擎、正在生成的报告、排队工作及尚未完成或无法确认的队列清理均会阻止删除；请先完成或停止相关工作并处理清理问题，再重试。详见[批量删除操作说明](docs/v1.3.5.md#overview-selection-and-bulk-deletion)。
...
捕获使用独立且固定版本的 mitmproxy 镜像。安装、浏览器信任、范围规则、存储及当前限制见 [MCP 操作指南](docs/mcp-traffic-workbench.md)，升级命令见 [1.3.5 版本说明](docs/v1.3.5.md)。
...
上面的克隆命令选择持续维护的 `v1.3.5-release` 分支，`release` 是分支名称的一部分。
如需固定历史快照，可使用保留的 `v1.3.5` 标签，其完整引用为 `refs/tags/v1.3.5`；该快照不含本次总览选择与批量删除回补，请拉取 `v1.3.5-release` 获取此功能。

升级已有项目之前，请先保存本地修改，包括编辑过的 prompt 和 skill。
等待工作结束，或先停止工作，再切换源码版本：
...
由 systemd 或其他服务管理器启动的服务，请先通过原管理器停止。
安装后刷新浏览器。如果本地修改或分叉提交导致切换或快进更新失败，请先保存并处理这些改动，再重试。
在 `main` 上执行 `git pull` 会更新开发代码。

从[稳定分支](https://github.com/0xG1w4/StrixOps/tree/v1.3.5-release)下载的归档包含仍需构建的源码。
解压并进入目录后，仍需执行 `./strixops.sh install --build-images` 和
`./strixops.sh start`。已有所需沙箱镜像时，使用普通 `install` 即可。
...
从本分支自行构建的 wheel 包包含已构建的控制台和运行时提示词、技能资源库。
按照[打包](#packaging)步骤构建后，使用以下命令安装 `dist/` 中的产物。
Docker 沙箱镜像需单独部署；以下步骤不假定包已发布至 PyPI。
...
**测试深度**分为 Default 和 Deep。Web Default 为主代理和子代理预载 Standard 方法论，覆盖业务流程、角色边界和系统性测试；主代理负责协调，子代理只执行分配范围内的检查。内网 Default 保留现有内网方法论。Deep 加载独立的深度测试方法论，不叠加 Standard，也不改变模型、推理强度或运行资源上限。
...
请使用能够绑定挂载引擎工作区路径的 Docker 守护进程。仅设置远程 `DOCKER_HOST`，不会让本地工作区目录自动出现在远程主机上。产品版本 `1.3.5` 与沙箱标签 `1.3.0` 是各自独立的版本号。
...
- **笔记 → 凭据**汇总 Agent 登记、明确的结构化凭据字段（例如 `metadata.credentials` 中实际的对象或列表）及明确标识的凭据 CSV，支持搜索、验证状态筛选和完整 CSV 下载。普通发现或笔记正文、URL 示例、Markdown 表格或代码块，以及正文中的 JSON 字符串都不会生成凭据条目；漏洞、内部发现、共享笔记、测试覆盖和威胁模型仍作为报告上下文保留。
- 升级后重新读取凭据清单时，以前仅从普通文字推测出的条目会消失，不会删除已登记的数据。已保存的报告不会自动修改，需要在任务页点击**生成报告**重新生成。登记、批量导入与报告来源规则见[管理指南](docs/management.md)。
- 报告为标题、段落、编号及嵌套列表提供明确间距；宽表格、代码块及受支持的任务流程图在各自区域内滚动。既有报告的 Markdown 和下载内容不会被重写。
- 共用的 `reporting/report_format` 技能向主智能体、子智能体及最终合成提供格式指导。新报告可在句子边界拆分较长的纯文本段落，同时保留必需内容与字面语法。格式处理不会新增模型调用，也不改变任务成功／失败规则。
- 捕获证据前会停止智能体和沙箱写入进程。复制文件时以流式方式计算 SHA256，没有固定的 50 MiB 截断限制；工作区原文件与归档副本可能同时占用磁盘空间。
- 证据收集采用尽力保存策略。证据列表及报告附件清单仅展示已保存且可用的附件；缺失路径引用不会让原本成功的任务失败。收集与复制问题仍保留在运行元数据中供诊断。智能体／运行时失败、容器清理错误，以及必要运行状态或报告保存失败，仍会导致任务失败。
- 升级后启动的新任务采用此策略，已有失败记录不会自动改写。重启升级后的控制台，也会从旧任务证据列表中过滤不可用附件。
...
重试次数有上限，且仅适用于符合条件的暂时性故障。输出开始前，SDK 最多重试 5 次。无远端会话状态的模型在输出途中断线（例如 `RemoteProtocolError: incomplete chunked read`）时，智能体可从已保存的会话最多连续恢复 5 次，依次等待 2、4、8、16、32 秒。完成一次模型回合后，连续失败计数会重置，但恢复不会重置当前执行周期的模型回合上限。日志和对话会显示 `[model retry]`；点击停止也会取消重试等待。

恢复会保留已完成的工具结果，不会重复追加旧提示、恢复提醒或未完成的工具调用。SDK 耗尽输出前重试后，不会再开启第二轮重试预算。托管工具和远端会话状态不参与会话恢复；认证、计费、明确的策略拒绝及工具或存储错误也不适用。通用的流式 `500` 可能掩盖更具体的上游拒绝；若暂时性故障持续，仍会在耗尽恢复次数后结束智能体。
