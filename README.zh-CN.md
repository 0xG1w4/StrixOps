...
版本 **1.1.3** · [版本说明](docs/v1.1.3.md) · [更新日志](CHANGELOG.md) · [Apache-2.0](LICENSE)
...
捕获使用独立且固定版本的 mitmproxy 镜像。安装、浏览器信任、范围规则、存储及当前限制见 [MCP 操作指南](docs/mcp-traffic-workbench.md)，升级命令见 [1.1.3 版本说明](docs/v1.1.3.md)。
...
```bash
git clone --branch v1.1.3 https://github.com/0xG1w4/StrixOps.git
cd StrixOps
...
上面的克隆命令选择 `v1.1.3` **发布标签**，以 detached HEAD 状态打开该版本的精确快照，不是持续维护的发布分支。在已有仓库中，该标签的完整引用为 `refs/tags/v1.1.3`。
...
wheel 包包含已构建的控制台和运行时提示词、技能资源库，**不包含** Docker 沙箱镜像。如果你已持有可信的 `strixops-1.1.3-py3-none-any.whl`，可以采用此方式。以下步骤不假定包已发布至 PyPI，也不假定 GitHub Release 已上传安装文件。自行构建的方法见[打包](#packaging)。

```bash
python3 -m venv .venv
.venv/bin/python -m pip install ./strixops-1.1.3-py3-none-any.whl
...
请使用能够绑定挂载引擎工作区路径的 Docker 守护进程。仅设置远程 `DOCKER_HOST`，不会让本地工作区目录自动出现在远程主机上。产品版本 `1.1.3` 与沙箱标签 `1.3.0` 是各自独立的版本号。
