...
wheel 包包含已构建的控制台和运行时提示词、技能资源库，**不包含** Docker 沙箱镜像。如果你已持有可信的 `strixops-1.3.5-py3-none-any.whl`，可以采用此方式。以下步骤不假定包已发布至 PyPI，也不假定 GitHub Release 已上传安装文件。自行构建的方法见[打包](#packaging)。
...
从 `main` 构建的开发版 wheel 在元数据与文件名中使用 Python 规范化版本 `1.4.0.dev1`；CLI 和控制台显示 `1.4.0-dev.1`。
...
请使用能够绑定挂载引擎工作区路径的 Docker 守护进程。仅设置远程 `DOCKER_HOST`，不会让本地工作区目录自动出现在远程主机上。开发产品版本 `1.4.0-dev.1` 与沙箱标签 `1.3.0` 是各自独立的版本号。
