...
function TaskDirectory() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      catch (e) { if (!disposed && controller === current && !document.hidden) setError(current.signal.aborted ? c("读取任务超时，请重试。", "Loading tasks timed out. Retry to reconnect.") : mcpError(e)); }
  ...
  return <>
    <header className={styles.pageHeader}><div><div className={styles.eyebrow}><Waypoints size={14} /> MCP / TRAFFIC</div><h1>MCP</h1><p>{c("将浏览器流量转成可探索的页面、API 与测试证据。", "Turn browser traffic into discoverable pages, APIs, and test evidence.")}</p></div><div className={styles.actionGroup}><button type="button" className={styles.iconButton} aria-label={c("刷新", "Refresh")} onClick={() => setRevision(v => v + 1)}><RefreshCw size={16} /></button><button type="button" className={styles.secondaryButton} aria-expanded={caOpen} onClick={() => setCaOpen(value => !value)}><ShieldCheck size={15} />{c("共享 CA", "Shared CA")}</button><button type="button" className={styles.primaryButton} onClick={() => setCreateOpen(true)}><Plus size={16} />{c("创建任务", "Create task")}</button></div></header>
    {unavailableNotice && <div className={styles.formNote} role="status">{c("任务已删除或无法再取得；请从下方选择其他任务。", "The task was deleted or is no longer available. Choose another task below.")}</div>}
    ...
    <section className={styles.directory} aria-label={c("MCP 任务", "MCP tasks")}>
      <div className={styles.sectionHeader}><h2>{c("独立流量任务", "Traffic tasks")}</h2><span>{tasks.length} {c("个任务", "tasks")}</span></div>
      {loading ? <div className={styles.contentEmpty}><LoaderCircle size={28} className={styles.spin} /><p>{c("读取任务…", "Loading tasks…")}</p></div> : !tasks.length ? <div className={styles.directoryEmpty}><div className={styles.emptyGlyph}><Network size={34} /></div><h2>{c("从一段浏览流程开始", "Start with a browsing session")}</h2><p>{c("创建任务并启动代理，浏览目标网站后，即可从捕获的请求展开测试。", "Create a task, start its proxy, and browse your target. Use the captured requests to begin testing.")}</p><div className={styles.emptySteps}><span>01 {c("连接代理", "Connect proxy")}</span><ArrowRight size={14} /><span>02 {c("探索流量", "Explore traffic")}</span><ArrowRight size={14} /><span>03 {c("测试与报告", "Test and report")}</span></div><button className={styles.primaryButton} onClick={() => setCreateOpen(true)}><Plus size={15} />{c("创建第一个任务", "Create your first task")}</button></div> : <div className={styles.taskRows}>{tasks.map(task => <div className={styles.taskRowShell} key={task.id}><Link href={`/mcp?task=${encodeURIComponent(task.id)}`} className={styles.taskRow}><div className={styles.taskGlyph}><Waypoints size={21} /></div><div className={styles.taskIdentity}><div><h3 title={task.name}>{task.name}</h3><McpStatus status={task.status} /></div><p title={task.allow_hosts.join(" · ")}>{task.allow_hosts.join(" · ") || c("尚未设置主机", "No hosts configured")}</p></div><div className={styles.taskCounts}><span><strong>{task.counts?.flows ?? 0}</strong>{c("请求", "requests")}</span><span><strong>{task.counts?.tests ?? 0}</strong>{c("测试", "tests")}</span></div><time>{dateText(task.updated_at)}</time><ArrowRight size={16} className={styles.rowArrow} /></Link><button type="button" className={styles.deleteRowButton} disabled={task.status === "deleting"} aria-label={c(`删除任务 ${task.name}`, `Delete task ${task.name}`)} onClick={() => setDeleteTask(task)}><Trash2 size={15} /></button></div>)}</div>}
    </section>
...
function TaskWorkspace({ taskId, catalog, catalogError }: { taskId: string; catalog: McpCatalog | null; catalogError: string }) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      if (current.signal.aborted) { setError(c("更新超时，正在显示上次取得的数据。", "Update timed out. Showing the last available data.")); }
      else {
  ...
  if (!task) return <><Link href="/mcp" className={styles.backLink}><ArrowLeft size={14} />{c("返回 MCP", "Back to MCP")}</Link>{error ? <McpError onRetry={refreshAll}>{error}</McpError> : <div className={styles.contentEmpty}><LoaderCircle size={28} className={styles.spin} /><p>{c("读取工作台…", "Loading workspace…")}</p></div>}</>;
  ...
  return <>
    <Link href="/mcp" className={styles.backLink}><ArrowLeft size={14} />{c("所有 MCP 任务", "All MCP tasks")}</Link>
    <header className={styles.pageHeader}><div><div className={styles.eyebrow}>MCP / <span className={styles.mono}>{shortId(task.id)}</span></div><h1>{task.name}</h1><div className={styles.taskSubtitle}><McpStatus status={task.status} /><span>{task.allow_hosts.join(" · ")}</span></div></div><div className={styles.actionGroup}><button className={styles.iconButton} type="button" aria-label={c("刷新", "Refresh")} onClick={refreshAll}><RefreshCw size={15} /></button><button className={styles.secondaryButton} disabled={ended || mutationBlocked} onClick={() => setSettingsOpen(true)}><Settings2 size={15} />{c("设置", "Settings")}</button>{!ended && <button className={styles.secondaryButton} disabled={mutationBlocked} onClick={() => setEndOpen(true)}><CheckCheck size={15} />{c("结束任务", "End task")}</button>}<button type="button" className={styles.deleteButton} disabled={busy} onClick={() => setDeleteOpen(true)}><Trash2 size={14} />{task.status === "delete_failed" ? c("重试删除", "Retry delete") : c("删除", "Delete")}</button></div></header>
    {(error || dataError || actionError) && <McpError onRetry={refreshAll}>{actionError || error || `${c("部分数据无法更新", "Some data could not be updated")}: ${dataError}`}</McpError>}
    <section className={styles.sessionBar} aria-label={c("代理会话", "Proxy session")}><div className={styles.sessionIdentity}><span className={styles.sessionSignal} data-live={capturing}><Network size={18} /></span><div><div className={styles.sessionTitle}>{capturing ? c("代理正在捕获流量", "Proxy is capturing traffic") : task.status === "error" ? c("代理需要处理", "Proxy needs attention") : ended ? c("任务已结束，数据已保留", "Task ended; records retained") : c("启动代理，开始观察网站", "Start the proxy to observe your site")}</div><div className={styles.sessionMeta}>{session?.proxy_port ? <code>{session.proxy_host || "127.0.0.1"}:{session.proxy_port}</code> : <span>{c("每个会话使用独立容器", "Each session uses an isolated container")}</span>}{session && <span>SESSION #{shortId(session.id)}</span>}</div></div></div><div className={styles.actionGroup}><button className={styles.secondaryButton} onClick={() => setConnectionOpen(value => !value)} aria-expanded={connectionOpen}><ShieldCheck size={14} />{c("连接方式", "Connection")}<ChevronDown size={13} /></button>{task.status === "error" && session && <button className={styles.secondaryButton} disabled={mutationBlocked} onClick={() => void transition("stop")}><Square size={13} />{c("停止代理", "Stop proxy")}</button>}{!ended && <button className={capturing ? styles.secondaryButton : styles.primaryButton} disabled={mutationBlocked} onClick={() => void transition(capturing ? "stop" : "start")}>{busy ? <LoaderCircle size={14} className={styles.spin} /> : capturing ? <Square size={13} /> : <Play size={14} />}{busy ? c("处理中…", "Working…") : capturing ? c("停止捕获", "Stop capture") : c("启动代理", "Start proxy")}</button>}</div></section>
    {proxyError && <McpError>{proxyError}</McpError>}
    {connectionOpen && <ConnectionPanel task={task} />}
    <div className={styles.workspaceTabs} role="tablist" aria-label={c("工作台内容", "Workspace content")}>{([{ id: "traffic", label: c("流量", "Traffic"), icon: Network, count: task.counts?.flows }, { id: "tests", label: c("Agent 测试", "Agent tests"), icon: FlaskConical, count: tests.length }, { id: "reports", label: c("报告", "Reports"), icon: FileText, count: reports.length }] as const).map(item => <button type="button" key={item.id} role="tab" id={`mcp-workspace-tab-${item.id}`} aria-selected={tab === item.id} aria-controls={`mcp-workspace-${item.id}`} onClick={() => setTab(item.id)}><item.icon size={15} />{item.label}{item.count != null && <span>{item.count}</span>}</button>)}</div>
    ...
    {endOpen && <McpModal title={c("结束这个 MCP 任务？", "End this MCP task?")} description={c("代理将停止，任务不再接收新流量与测试。现有数据仍可查看并生成报告。", "The proxy stops accepting traffic and new tests. Existing records remain available for review and reporting.")} onClose={() => !action && setEndOpen(false)}><div className={styles.form}>{activeTests(tests) && <div className={styles.warningNote}>{c("仍有 Agent 测试进行中；后端会处理任务结束时的执行状态。", "Agent tests are still active; the server will handle their state when ending the task.")}</div>}{actionError && <McpError>{actionError}</McpError>}<div className={styles.formActions}><button className={styles.secondaryButton} disabled={Boolean(action)} onClick={() => setEndOpen(false)}>{c("继续工作", "Keep working")}</button><button className={styles.primaryButton} disabled={Boolean(action)} onClick={() => void transition("end")}>{action ? c("结束中…", "Ending…") : c("结束任务", "End task")}</button></div></div></McpModal>}
  </>;
}
...
function TrafficWorkspace({ task, live, active, endpoints, refresh, selectedId, onSelect, onTest, onRefresh, onTests }: { task: McpTask; live: boolean; active: boolean; endpoints: McpEndpoint[]; refresh: number; selectedId: string | null; onSelect: (id: string | null) => void; onTest: (ids: string[]) => void; onRefresh: () => void; onTests: () => void }) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (e) { if (!disposed && controller === current && !document.hidden) setFlowError(current.signal.aborted ? c("流量更新超时；已保留画面中的数据。", "Traffic update timed out; existing rows are retained.") : mcpError(e)); }
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      catch (e) { if (!disposed && controller === current && !document.hidden) setDetailError(current.signal.aborted ? c("读取内容超时，请重新选择请求。", "Loading content timed out. Select the request again.") : mcpError(e)); }
  ...
  return <>
    <div className={styles.trafficToolbar}><div className={styles.searchField}><Search size={15} /><input aria-label={c("搜索流量", "Search traffic")} value={query} onChange={e => setQuery(e.target.value)} placeholder={c("搜索 URL、主机或请求…", "Search URL, host, or request…")} /></div><label className={styles.sourceFilter}><span>{c("来源", "Source")}</span><select value={source} onChange={e => setSource(e.target.value)} aria-label={c("请求来源", "Request source")}><option value="">{c("全部来源", "All sources")}</option><option value="user">{c("浏览器", "Browser")}</option><option value="replay">{c("重放", "Replay")}</option><option value="agent">Agent</option></select></label><button className={styles.secondaryButton} disabled={!checked.length || !mutable} onClick={() => onTest(checked)}><FlaskConical size={14} />{c("测试所选请求", "Test selected")}{checked.length > 0 && <span>{checked.length}</span>}</button></div>
    {flowError && <McpError onRetry={onRefresh}>{flowError}</McpError>}
    <div className={styles.trafficGrid}>
      <aside className={styles.siteTree}><div className={styles.treeHeading}><Globe2 size={14} /><h3>{c("已观察的网站", "Observed sites")}</h3></div><div className={styles.kindFilters}>{[{ id: "", label: c("全部请求", "All traffic"), count: task.counts?.flows }, { id: "page", label: c("页面", "Pages"), count: task.counts?.pages }, { id: "api", label: "API", count: task.counts?.apis }, { id: "asset", label: c("静态资源", "Assets") }, { id: "other", label: c("其他", "Other") }].map(item => <button key={item.id} type="button" aria-pressed={kind === item.id} onClick={() => setKind(item.id)}><span>{item.label}</span>{item.count != null && <small>{item.count}</small>}</button>)}</div><div className={styles.hostInventory}>{grouped.map(([host, entries]) => <details key={host} open><summary title={host}><Globe2 size={12} /><span>{host}</span><small>{entries.length}</small></summary><div>{entries.map((endpoint, index) => <button type="button" key={endpoint.id || `${endpoint.method}:${endpoint.path}:${index}`} className={styles.endpoint} title={`${endpoint.method} ${endpoint.host}${endpoint.path}`} aria-label={`${endpoint.method} ${endpoint.host}${endpoint.path}`} onClick={() => { setQuery(`${host}${endpoint.path.split("{id}")[0]}`); if (endpoint.latest_flow_id) onSelect(endpoint.latest_flow_id); }}><span className={styles.endpointMethod}>{endpoint.method}</span><span>{endpoint.path}</span><small>{endpoint.count ?? endpoint.flow_count ?? ""}</small></button>)}</div></details>)}{!grouped.length && <p className={styles.treeEmpty}>{c("有流量后，这里会创建页面与 API 目录。", "Pages and APIs appear here as traffic arrives.")}</p>}</div><div className={styles.treeFootnote}><Layers3 size={12} />{c("端点由已观察路径分组", "Grouped from observed paths")}</div></aside>
      <section className={styles.flowList} aria-label={c("捕获的请求", "Captured requests")}><div className={styles.flowListHeader}><label><input type="checkbox" aria-label={c("选择当前列表的所有请求", "Select all visible requests")} checked={flows.length > 0 && flows.every(flow => checked.includes(flow.id))} onChange={e => setChecked(e.target.checked ? flows.map(flow => flow.id) : [])} /><span>{c("请求", "Requests")}</span></label><span>{flows.length} / {total}</span></div>
        {!flows.length ? <div className={styles.flowEmpty}>{loading ? <LoaderCircle size={24} className={styles.spin} /> : <Network size={26} />}<h3>{loading ? c("读取流量…", "Loading traffic…") : query || kind || source ? c("没有符合的请求", "No matching requests") : c("等待第一条流量", "Waiting for traffic")}</h3><p>{query || kind || source ? c("调整搜索或筛选条件。", "Adjust your search or filters.") : c("将浏览器连到代理，再打开范围内的网站。", "Connect your browser to the proxy, then open a site in scope.")}</p></div> : <div className={styles.flowRows}>{flows.map(flow => <div className={styles.flowRow} key={flow.id} data-selected={selectedId === flow.id}><label className={styles.flowCheckbox}><input aria-label={c(`选择请求 ${flow.method} ${flow.path}`, `Select request ${flow.method} ${flow.path}`)} type="checkbox" checked={checked.includes(flow.id)} onChange={e => setChecked(old => e.target.checked ? [...old, flow.id] : old.filter(id => id !== flow.id))} /></label><button type="button" className={styles.flowSelect} title={`${flow.method} ${flow.url}`} aria-label={`${flow.method} ${flow.url}`} aria-pressed={selectedId === flow.id} onClick={() => { onSelect(flow.id); if (selectedId === flow.id) setDetailRevision(value => value + 1); }}><span className={styles.method} data-method={flow.method}>{flow.method}</span><span className={styles.flowIdentity}><span>{flow.path || "/"}</span><small>{flow.host} · {dateText(flow.created_at, true)}</small></span><span className={styles.flowState}><span data-error={Boolean(flow.error) || Number(flow.status_code) >= 400}>{flow.status_code ?? (flow.error ? "ERR" : "…")}</span><small>{flow.source === "user" ? "Browser" : flow.source === "replay" ? "Replay" : "Agent"}</small></span></button></div>)}</div>}
        {nextCursor && <button className={styles.loadMore} disabled={moreLoading} onClick={() => void loadMore()}>{moreLoading ? c("加载中…", "Loading…") : c("加载较早的请求", "Load earlier requests")}<ChevronDown size={13} /></button>}
      </section>
