...
Version **1.2.3** · [Release notes](docs/v1.2.3.md) · [Changelog](CHANGELOG.md) · [Apache-2.0](LICENSE)
...
| [Shared task notes](docs/shared-notes.md) | Web/Internal Agent notebooks, version checks, atomic additions, retained history, and a read-only Notes tab | Reuse task discoveries across agents without overwriting each other's updates |
...
[1.2.3 release notes](docs/v1.2.3.md) for upgrade commands.
...
```bash
git clone --branch v1.2.3 https://github.com/0xG1w4/StrixOps.git
cd StrixOps
...
The clone command selects the `v1.2.3` **release tag**, leaving a detached HEAD
at that release snapshot. It does not select a maintained release branch.
In an existing checkout, the exact reference is `refs/tags/v1.2.3`.
...
trusted `strixops-1.2.3-py3-none-any.whl`; it does not assume a PyPI publication or
an uploaded GitHub Release asset. See [Packaging](#packaging) to build one.

```bash
python3 -m venv .venv
.venv/bin/python -m pip install ./strixops-1.2.3-py3-none-any.whl
...
prompt/skill resources are saved at submission; project scope is rechecked for
each target before it starts. Existing historical shared reports remain readable.
See the [FOFA and batch guide](docs/fofa-and-target-batches.md) for the workflow,
limits, storage and recovery behavior.
...
Multiple targets, including a list file, form a batch with independent reports:
...
| `--max-concurrent` | Concurrent targets per batch, 1–16; default 2 |
...
nonzero code, with details in the run directory. `--resume-batch` supervises
the remaining targets of a CLI batch; it does not restore an interrupted Agent
conversation. An ordinary rerun starts a new assessment.
...
that remote host. The product version `1.2.3` and sandbox tag `1.3.0` are separate
version numbers.
...
- The shared `reporting/report_format` skill supplies formatting guidance to root
  ...
  Formatting adds no model calls and does not change task success/failure rules.
