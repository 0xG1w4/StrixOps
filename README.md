...
Version **1.1.9** · [Release notes](docs/v1.1.9.md) · [Changelog](CHANGELOG.md) · [Apache-2.0](LICENSE)
...
[1.1.9 release notes](docs/v1.1.9.md) for upgrade commands.
...
```bash
git clone --branch v1.1.9 https://github.com/0xG1w4/StrixOps.git
cd StrixOps
...
The clone command selects the `v1.1.9` **release tag**, leaving a detached HEAD
at that release snapshot. It does not select a maintained release branch.
In an existing checkout, the exact reference is `refs/tags/v1.1.9`.
...
trusted `strixops-1.1.9-py3-none-any.whl`; it does not assume a PyPI publication or
an uploaded GitHub Release asset. See [Packaging](#packaging) to build one.

```bash
python3 -m venv .venv
.venv/bin/python -m pip install ./strixops-1.1.9-py3-none-any.whl
...
that remote host. The product version `1.1.9` and sandbox tag `1.3.0` are separate
version numbers.
