...
Version **1.1.3** · [Release notes](docs/v1.1.3.md) · [Changelog](CHANGELOG.md) · [Apache-2.0](LICENSE)
...
[1.1.3 release notes](docs/v1.1.3.md) for upgrade commands.
...
The clone command selects the `v1.1.3` **release tag**, leaving a detached HEAD
at that release snapshot. It does not select a maintained release branch.
In an existing checkout, the exact reference is `refs/tags/v1.1.3`.
...
trusted `strixops-1.1.3-py3-none-any.whl`; it does not assume a PyPI publication or
...
that remote host. The product version `1.1.3` and sandbox tag `1.3.0` are separate
version numbers.
